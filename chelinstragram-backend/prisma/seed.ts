...
async function main() {
    ...
    const hashedPassword = await bcrypt.hash('!Q2w3e4r5', 10);
    ...
    const official = await prisma.user.upsert({
        ...
        create: {
            ...
            avatarUrl: "/uploads/chelinstagram.png"
        },
    });
    ...
    for (const postData of postsToSeed) {
        try {
            ...
            const cleanFileName = postData.mediaUrl.replace('/uploads/', '');
            const filePath = path.join(imagesDir, cleanFileName);

            // 2. Verificar que el archivo realmente exista en tu entorno local
            if (!fs.existsSync(filePath)) {
                console.warn(`⚠️ Skipping: File not found at ${filePath}`);
                continue;
            }

            // 3. Guardar en Prisma apuntando a tu servidor local de archivos
            await prisma.post.create({
                data: {
                    authorId: postData.authorId,
                    mediaUrl: `/uploads/${cleanFileName}`,   // 👈 RUTA LOCAL DIRECTA
                    mediaType: 'IMAGE',
            ...
            console.log(`✅ Seeded (Local Storage): ${cleanFileName}`);
        } catch (err) {
